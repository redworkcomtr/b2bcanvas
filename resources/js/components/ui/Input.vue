<script setup lang="ts">
import { useSlots } from 'vue';

defineProps<{
    label?: string;
    modelValue?: string | number;
    type?: string;
    placeholder?: string;
    required?: boolean;
    help?: string;
    error?: string;
    disabled?: boolean;
    maxlength?: number | string;
}>();

const emit = defineEmits<{
    'update:modelValue': [value: string];
}>();

const slots = useSlots();
</script>

<template>
    <label class="grid min-w-0 gap-1.5 text-[14px] font-medium text-zinc-700">
        <span v-if="label" class="text-[12px] font-semibold text-zinc-600">{{ label }} <span v-if="required" class="text-red-600">*</span></span>
        <div class="relative min-w-0">
            <span v-if="slots.prefix" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 text-zinc-400">
                <slot name="prefix" />
            </span>
            <input
                :class="[
                    'focus-ring h-10 w-full rounded-md border bg-white px-3 text-[14px] text-zinc-950 shadow-[inset_0_1px_1px_rgba(15,23,42,0.03)] placeholder:text-zinc-400',
                    slots.prefix ? 'pl-9' : '',
                    error ? 'border-red-300' : 'border-zinc-300/80',
                    disabled ? 'cursor-not-allowed opacity-60' : '',
                ]"
                :type="type ?? 'text'"
                :value="modelValue"
                :placeholder="placeholder"
                :disabled="disabled"
                :maxlength="maxlength"
                @input="emit('update:modelValue', ($event.target as HTMLInputElement).value)"
            >
        </div>
        <span v-if="error" class="text-xs font-medium text-red-600">{{ error }}</span>
        <span v-else-if="help" class="text-xs font-normal text-zinc-500">{{ help }}</span>
    </label>
</template>
