    ...
    <div class="app-page">
        <div class="flex flex-col justify-between gap-3 lg:flex-row lg:items-end">
            <div class="page-heading">
                ...
                <p>{{ store.tenant?.name ?? 'Workspace' }} · {{ store.user?.role ?? 'user' }}</p>
            </div>
        ...
        <div class="flex flex-wrap items-center justify-between gap-3">
            ...
            <span class="text-xs font-medium text-[#71717a]">{{ dashboardProfile === 'operations' ? 'Operations queue' : 'Executive rollup' }}</span>
        </div>
        ...
        <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <Card>
                <h3 class="panel-title mb-1">Order status</h3>
                <p class="panel-caption mb-4">Fulfillment lifecycle mix</p>
                <div class="grid gap-3">
                    <div v-for="item in statusDistribution" :key="item.key">
                        <div class="mb-1 flex items-center justify-between text-sm text-[#4c4546]">
                            <span class="font-semibold text-[#18181b]">{{ item.label }}</span>
                            <span>{{ item.value }}</span>
                        </div>
                        <div class="h-2 rounded-full bg-slate-100">
                            <div class="h-2 rounded-full bg-[#18181b]" :style="{ width: `${Math.min(item.ratio, 100)}%` }" />
                        </div>
                    </div>
                </div>
            </Card>
            ...
            <Card>
                <h3 class="panel-title mb-1">Import health</h3>
                <p class="panel-caption mb-4">Batch and row quality</p>
                <div class="grid gap-4">
                    <div>
                        <p class="text-sm font-semibold text-[#18181b]">Batch states</p>
                    ...
                    <div>
                        <p class="text-sm font-semibold text-[#18181b]">Rows</p>
                        <p class="mt-2 text-sm text-[#4c4546]">Ready {{ importHealth.rows.ready }} · Committed {{ importHealth.rows.committed }}</p>
                        <p class="text-sm text-[#4c4546]">Needs action {{ importHealth.rows.needsAction }} · Total {{ importHealth.rows.total }}</p>
                    </div>

                    <div>
                        <p class="text-sm font-semibold text-[#18181b]">Actionable import ratio</p>
                        <div class="mt-2 h-2 rounded-full bg-slate-100">
                            <div class="h-2 rounded-full bg-emerald-600" :style="{ width: `${importHealth.actionRate}%` }" />
                        </div>
                        <p class="mt-1 text-xs text-[#71717a]">{{ importHealth.actionRate }}% of import rows are actionable</p>
                    </div>
                </div>
            </Card>

            <Card>
                <h3 class="panel-title mb-1">Operations snapshot</h3>
                <p class="panel-caption mb-4">Current operator pressure</p>
                <div class="grid gap-3">
                    <div>
                        <p class="text-sm font-semibold text-[#18181b]">Unpaid vs Action needed</p>
                        <p class="text-sm text-[#4c4546]">{{ operationalMetrics.paymentUnpaid }} unpaid · {{ metrics.orders_open_action_needed ?? 0 }} action-needed</p>
                    </div>
                    <div>
                        <p class="text-sm font-semibold text-[#18181b]">Active work items</p>
                    ...
                    <div>
                        <p class="text-sm font-semibold text-[#18181b]">Open production</p>
                        <p class="text-sm text-[#4c4546]">{{ operationalMetrics.inProduction }} orders in production flow.</p>
                    </div>
                    <div class="rounded-lg border border-zinc-200 bg-white px-3 py-2 text-xs text-zinc-500">
                        <p class="font-semibold text-[#18181b]">Import row quality</p>
            ...
            <Card>
                <div class="mb-4 flex items-center justify-between gap-4">
                    <div>
                        ...
                        <p class="panel-caption">Recent customer activity</p>
                    </div>
                    <RouterLink to="/orders">
                        <Button variant="outline" size="sm">
                ...
                <DataTable min-width="900px">
                    ...
                    <tbody class="divide-y divide-zinc-100">
                        <tr v-for="order in latestOrders" :key="order.id">
                            ...
                            <td class="px-4 py-3 text-[#4c4546]">{{ order.customer_name }}</td>
                            <td class="px-4 py-3">
                                <Badge :tone="order.payment_status === 'paid' || order.payment_status === 'not_required' ? 'success' : 'warning'">
                                    {{ order.payment_status ?? 'pending' }}
                                </Badge>
                            </td>
                            <td class="px-4 py-3 text-[#71717a]">{{ dateLabel(order.submitted_at) }}</td>
                            <td class="px-4 py-3 text-[#4c4546]">{{ order.items?.length ?? 0 }}</td>
                        </tr>
                    </tbody>
                </DataTable>
            </Card>
            ...
            <Card>
                <div class="mb-4 flex items-center justify-between">
                    <div>
                        ...
                        <p class="panel-caption">Production blockers</p>
                    </div>
                ...
                <div class="grid gap-3">
                    <RouterLink
                        ...
                        :to="item.to"
                        class="rounded-lg border border-zinc-200 bg-white p-3 transition-colors hover:bg-zinc-50"
                    >
                        <div class="flex items-center justify-between gap-2">
                            <p class="font-semibold text-[#18181b]">{{ item.label }}</p>
                            <Badge :tone="item.tone">Open</Badge>
                        </div>
                        <p class="mt-1 line-clamp-2 text-sm text-[#4c4546]">{{ item.caption }}</p>
                    </RouterLink>
